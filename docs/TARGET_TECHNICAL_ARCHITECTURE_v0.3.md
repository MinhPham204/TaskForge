...
| TA-08 | Application-level explicit tenant scoping là v1 correctness boundary; RLS để hardening sau | RLS bắt buộc ngay; implicit ORM filter | Giảm Prisma/pooling complexity ban đầu; vẫn giữ composite FK và integration tests | Deferred hardening |
...
2. Module export Service/query contract hẹp; không export raw Prisma client hoặc repository để module khác query tùy ý.
...
| Default | Exact module nào enabled khi tạo Project chưa được scope khóa: `TBD-MOD-01`; không dùng absence của row như business state mơ hồ |
...
| Lifecycle | Scope yêu cầu lifecycle và close/reopen semantics nhưng chưa khóa exact enum: `TBD-MILE-01` |
...
| TBD | Scale của likelihood/impact và eligibility chính xác của owner: `TBD-RISK-01` |
...
- Exact transition vào/ra `ARCHIVED` chưa được Business Scope khóa đầy đủ: `TBD-PROJ-02`.
...
**Quyết định: PostgreSQL là persistence duy nhất của target architecture; Prisma hoặc ORM được chọn chỉ là infrastructure detail.**
...
Quyết định này không phải authorization tạo Prisma schema/migration trong task hiện tại. Target Domain Model và ADR persistence/identifier phải được review trước schema đầu tiên.
...
- Controller không thao tác Prisma trực tiếp; business rule không phụ thuộc Prisma-specific behavior.
...
- Tenant-scoped data access nhận `organizationId` tường minh và trả application/output model, không trả raw Prisma record qua API.
- QueryService có thể dùng Prisma/SQL tối ưu khi Board/List/Search khác rõ transactional CRUD, nhưng vẫn enforce tenant/visibility.
...
PostgreSQL RLS được xếp **LATER / security hardening**. Chỉ đánh giá triển khai sau khi schema, Prisma transaction/pooling và application tenant tests đã ổn định. Khi đó ADR riêng phải khóa runtime DB role, transaction-local tenant context, `USING`/`WITH CHECK` và system-operation bypass. Architecture Acceptance và v1 correctness không phụ thuộc RLS.
...
- Exact Milestone lifecycle enum là `TBD-MILE-01`.
...
- Probability/impact scale và detailed contributor ownership policy là `TBD-RISK-01`/`TBD-RISK-02`.
...
| TBD-MILE-01 | Exact Milestone lifecycle enum? | Milestone module |
| TBD-MOD-01 | Default enable/disable của bốn optional module khi tạo Project? | Project defaults/UI |
| TBD-RISK-01 | Likelihood/impact scale và Risk owner eligibility? | Risk schema/validation |
...
Tài liệu hiện tại không định nghĩa executable phase order, migration command, Prisma model hoặc source refactor. Mọi implementation phải trace về accepted Business Scope và một architecture/ADR decision đã được review tương ứng.
